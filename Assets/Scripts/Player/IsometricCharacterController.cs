...
public class IsometricCharacterController : MonoBehaviour
{
        ...
        switch(transformation) {
            ...
            case Transformation.BULLDOZER:
                if (isMoving) {
                    ...
                    if (!AudioManager.Instance.IsPlaying("Bulldozer Walk"))
                        AudioManager.Instance.Play("Bulldozer Walk");
                } else {
                    if (direction == Direction.DOWN) animator.Play(staticBulldozerDirections[0]);
                    else animator.Play(staticBulldozerDirections[1]);

                    if (AudioManager.Instance.IsPlaying("Bulldozer Walk"))
                        AudioManager.Instance.Pause("Bulldozer Walk");
                }
                break;
        }
    }
    ...
    public void Smoke() {
        ...
        AudioManager.Instance.Play("Transformation Poof");
    }
